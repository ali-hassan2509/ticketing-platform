        ...
        <div className="flex items-center gap-4">
          {user ? (
            <>
              {/* NEW ADMIN BUTTON */}
              {user.is_admin && (
                <Link to="/admin" className="text-sm font-bold bg-emerald-600/20 text-emerald-400 px-3 py-1 rounded-lg hover:bg-emerald-600/40 transition-colors">
                  🛡️ Admin Panel
                </Link>
              )}
              ...
              <div className="flex items-center gap-2 ml-2 pl-4 border-l border-gray-800">
                {user.picture && <img src={user.picture} alt="" className="w-8 h-8 rounded-full border border-gray-600" />}
                <span className="text-sm text-gray-300 hidden sm:block">{user.name}</span>
                <button onClick={handleLogout} className="text-xs text-gray-500 hover:text-red-400 ml-2">Sign out</button>
              </div>
            </>
          ) :  (
            <a
